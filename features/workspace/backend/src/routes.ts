...
import { clientForIntegration, fullSync, workspaceSlugOf } from "./sync/engine";
import { upsertComment, upsertWorkItem } from "./sync/upsert";
...
workspaceRoutes.post("/work-items/:id/comments", async (req, res, next) => {
  try {
    if (!req.user) {
    ...
    const workItem = await prisma.planeWorkItem.findUnique({
      ...
      select: {
        ...
        externalId: true,
        project: {
          select: { externalId: true, workspaceId: true, integrationId: true },
        },
      },
    });
    if (!workItem) {
    ...
    const commentHtml = `<p><strong>${escapeHtml(req.user.displayName)}:</strong> ${escapeHtml(body)}</p>`;
    let created;
    try {
      created = await client.createComment(slug, workItem.project.externalId, workItem.externalId, {
        comment_html: commentHtml,
      });
    } catch (err) {
      if (err instanceof PlaneApiError) {
        res.status(502).json({ error: `Plane rejected the comment (${err.status})` });
        return;
      }
      throw err;
    }
    const persisted = await prisma.$transaction((tx) => upsertComment(tx, workItem.id, created));
    const row = await prisma.planeComment.findUniqueOrThrow({ where: { id: persisted.id } });
    const authorMap = await hydrateCommentAuthors(workItem.project.workspaceId, [row]);
    res.json(
      toCommentDto(
        row,
        row.authorExternalId ? (authorMap.get(row.authorExternalId) ?? null) : null,
      ),
    );
  } catch (err) {
    next(err);
...
workspaceRoutes.patch("/work-items/:id", async (req, res, next) => {
  try {
    if (!req.user) {
      res.status(401).json({ error: "Not authenticated" });
      return;
    }
    const patch: { state?: string; assignees?: string[]; priority?: string } = {};
    if (typeof req.body?.stateExternalId === "string") patch.state = req.body.stateExternalId;
    if (Array.isArray(req.body?.assigneeExternalIds)) {
      patch.assignees = req.body.assigneeExternalIds.filter(
    ...
    const workItem = await prisma.planeWorkItem.findUnique({
      ...
      select: {
        ...
        externalId: true,
        projectId: true,
        project: { select: { externalId: true, integrationId: true } },
      },
    });
    if (!workItem) {
      res.status(404).json({ error: "Work item not found" });
    ...
    let updated;
    try {
      updated = await client.updateWorkItem(
        slug,
      ...
      if (err instanceof PlaneApiError) {
        res.status(502).json({ error: `Plane rejected the patch (${err.status})` });
        return;
      }
      throw err;
    }
    await prisma.$transaction((tx) => upsertWorkItem(tx, workItem.projectId, updated));
    res.json({ status: "ok" });
  } catch (err) {
    next(err);
  }
