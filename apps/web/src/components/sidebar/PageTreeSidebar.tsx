...
export function PageTreeSidebar() {
  ...
  return <SectionTree key={activeSection} section={activeSection} />;
}

interface CreateRequest {
  parentId: string | null;
  /** Forced when creating inside a folder — child must match folder's scope. */
  forceScope?: PageScope;
}

function SectionTree({ section }: { section: PageSection }) {
  const api = useApi();
  ...
  // Role-gate the My Approvals leaf: hide if the user is not admin and not a
  // lead anywhere. The polled summary returns canApprove for exactly this.
  // While summary is still loading we hide it conservatively rather than
  // flashing an entry the user can't act on.
  ...
  /** A user can edit (rename / delete / move) a page when it's their personal page or it's */
...
interface NodeRowProps {
  ...
  /** Optional pending-count lookup keyed by Page.url. */
  badgeForUrl?: (url: string | null) => number;
}

/** Maps Page.url → pending badge count for the Requests section. */
...
function CreatePageModal({
  ...
  // Picker first; once a kind is chosen, show the form.
  const [kind, setKind] = useState<"DASHBOARD" | "LINK" | "FOLDER" | null>(null);
  const [title, setTitle] = useState("");
  const [url, setUrl] = useState("");
  // Default to SHARED for admins (admin's normal flow is publishing to org);
  // non-admins always create personal pages so the toggle isn't shown.
            ...
            {kind === "LINK" && (
              <div className="flex flex-col gap-1">
                ...
                <UrlCombobox
                  ...
                  onChange={setUrl}
                  isAdmin={isAdmin}
                  // Auto-fill the title when the user picks a known route, but only
...
function UrlCombobox({
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (e.key === "Enter") {
      // If a suggestion is highlighted and the menu is open, accept it.
      // Otherwise let the parent form submit.
    ...
    <div ref={containerRef} className="relative">
      ...
      {open && matches.length > 0 && (
        <ul
          ...
          // Use mousedown so we register the pick before the input's onBlur fires.
